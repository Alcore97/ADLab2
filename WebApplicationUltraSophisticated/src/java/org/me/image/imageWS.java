...
public class imageWS {
    ...
    public List<Image> SearchbyCreaDate(@WebParam(name = "datacreacio")String datacreacio) {
      ...
      @WebMethod(operationName = "SearchbyId")
    public List<Image> SearchbyTitleandKeyword(@WebParam(name = "titol_imatge")String titol_imatge, @WebParam(name = "paraula_clau")String paraula_clau) {
        try {
            List<Image> llista = new ArrayList<>();
           connection = DriverManager.getConnection("jdbc:sqlite:C:\\Users\\aleix\\Desktop\\Escritorio\\pro2\\JavaMasterRace\\NetBeans\\LIBRERIA.db");
            PreparedStatement statement = connection.prepareStatement("select * from imatges where titol_imatge = ? and paraula_clau = ?");
            statement.setString(1,titol_imatge);
            statement.setString(2,paraula_clau);
            ResultSet rs = statement.executeQuery();
               while(rs.next()) {
                Image i1 = new Image();
                i1.SetIdImag(rs.getInt("id_imatge"));
